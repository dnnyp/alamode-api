      ...
      const productsOnPage = await page.evaluate(siteArguments =>
        Array.from(document.querySelectorAll(siteArguments.productSelector)).map(product => {
          ...
          return {
            ...
            url: product.querySelector(siteArguments.urlSelector).getAttribute('href'),
            price: product.querySelector(siteArguments.priceSelector).innerText,
            sale: sale ? sale.innerText : 'N/A'
          }
        }),
      siteArguments
      )
